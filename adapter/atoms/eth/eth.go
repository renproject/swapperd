...
func (atom *EthereumAtom) Initiate(to []byte, hash [32]byte, value *big.Int, expiry int64) error {
	...
	tx, err := atom.binding.Initiate(atom.key.TransactOpts, atom.data.SwapID, common.HexToAddress(string(to)), hash, big.NewInt(expiry))
...
func (atom *EthereumAtom) Audit() ([32]byte, []byte, *big.Int, int64, error) {
	details, err := atom.network.ReceiveSwapDetails(atom.orderID, time.Now().Add(15*time.Minute).Unix())
	if err != nil {
	...
	return auditReport.SecretLock, []byte(auditReport.To.String()), auditReport.Value, auditReport.Timelock.Int64(), nil
}
