...
1. Agree that one party will begin the atomic swap.  **This agreement must happen at the application level.**
2. Exchange their Swapperd addresses for the relevant tokens. **This exchange must happen at the application level.**

Before responding to an atomic swap, it is assumed that the two parties:

1. Exchange the `timeLock` and `secretHash` generated by the party that begins the atomic swap. **This exchange must happen at the application level.**
